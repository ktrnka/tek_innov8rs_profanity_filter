...
class SklearnProfanityFilter:
    ...
    def save_onnx(self, filepath: str, sample_input: str = "sample text"):
        ...
        try:
            ...
            click.echo(
                "ONNX export may not be supported for this model configuration.",
                err=True,
            )
...
@click.option(
    "--output",
    default="models/sklearn_filter.pkl",
...
def train(output, test_size):
    ...
    try:
        ...
        click.echo(
            f"Splitting data: {int((1 - test_size) * 100)}% train, {int(test_size * 100)}% test (stratified)..."
        )
        ...
        print_evaluation_report(
            y_test, y_pred, X_test, num_samples=10, filter_name="Sklearn Filter"
        )
...
@click.argument("text")
@click.option(
    "--model", default="models/sklearn_filter.pkl", help="Path to trained model"
)
...
@click.argument("text")
@click.option(
    "--model", default="models/sklearn_filter.onnx", help="Path to ONNX model"
)
def predict_onnx(text, model):
    ...
    Usage:
        ...
        uv run main.py sklearn predict-onnx "you noob" --model models/my_model.onnx
    """
    try:
        if not Path(model).exists():
            click.echo(f"Error: ONNX model not found at {model}", err=True)
            click.echo(
                "Train a model first with: uv run main.py sklearn train", err=True
            )
            raise click.Abort()
...
@click.option(
    ...
    "-n",
    default=10,
    help="Number of false positive/negative samples to show",
)
def evaluate(model, samples):
    ...
    try:
        ...
        print_evaluation_report(
            y_test, y_pred, X_test, num_samples=samples, filter_name="Sklearn Filter"
        )
...
@click.option(
    "--sample-size", default=10000, help="Number of usernames to evaluate (0 for all)"
)
@click.option(
    "--review-count", "-n", default=50, help="Number of flagged usernames to review"
)
def eval_usernames(model, sample_size, review_count):
    ...
    try:
        ...
        click.echo(
            "Note: This model was trained on gaming chat messages, not usernames.\n"
        )
        ...
        click.echo(
            f"Flagged as offensive: {flagged_count:,} ({flagged_count / len(usernames) * 100:.2f}%)"
        )
        click.echo(
            f"Clean: {clean_count:,} ({clean_count / len(usernames) * 100:.2f}%)"
        )
        ...
        if flagged_count > 0:
            ...
            click.echo(
                f"Showing {min(review_count, flagged_count)} of {flagged_count} total"
            )
            click.echo("=" * 60)
            click.echo(
                "Review these to estimate precision (true positives / flagged):\n"
            )
            ...
            click.echo("=" * 60)
            click.echo(
                "Manually count how many of the above usernames are actually offensive."
            )
            click.echo(
                f"Precision ≈ (offensive count) / {min(review_count, flagged_count)}"
            )
            click.echo("\nExpected offensive rate in general population: 0.1-5%")
            click.echo(
                f"If precision is high (>80%), estimated true offensive count: ~{int(flagged_count * 0.9):,}"
            )
            click.echo(
                f"If precision is medium (50%), estimated true offensive count: ~{int(flagged_count * 0.5):,}"
            )
